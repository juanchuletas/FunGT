...
fungl::Matrix3f::Matrix3f(float a00, float a01, float a02,
...
fungl::Matrix3f::Matrix3f(const Matrix3f &input)
{
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            m[i][j] = input.m[i][j];
...
fungl::Vec3 fungl::Matrix3f::operator*(const Vec3 &v) const
{
    return Vec3(
...
fungl::Matrix3f fungl::Matrix3f::operator*(const Matrix3f &other) const
{
     fungl::Matrix3f result;
...
fungl::Matrix3f fungl::Matrix3f::transpose() const
{
    fungl::Matrix3f result;
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
