#ifndef _PERLIN_NOISE_GPU_H_
#define _PERLIN_NOISE_GPU_H_

#include <cmath>

namespace PerlinGPU {

    // Permutation table - compile-time constant for GPU compatibility
    ...
    // 3D Perlin Noise (single octave)
    inline float pnoise3d(float x, float y, float z) {
        ...
        float lerp_u1 = lerp(u, grad(perm[AA], x, y, z), grad(perm[BA], x - 1, y, z));
        float lerp_u2 = lerp(u, grad(perm[AB], x, y - 1, z), grad(perm[BB], x - 1, y - 1, z));
        float lerp_u3 = lerp(u, grad(perm[AA + 1], x, y, z - 1), grad(perm[BA + 1], x - 1, y, z - 1));
        float lerp_u4 = lerp(u, grad(perm[AB + 1], x, y - 1, z - 1), grad(perm[BB + 1], x - 1, y - 1, z - 1));

        float lerp_v1 = lerp(v, lerp_u1, lerp_u2);
        float lerp_v2 = lerp(v, lerp_u3, lerp_u4);
        float result = lerp(w, lerp_v1, lerp_v2);

        return (result + 1.0f) / 2.0f; // Normalize to [0, 1]
    }

    // Multi-octave turbulence
    inline float turbulence3d(float x, float y, float z, int octaves = 4, float persistence = 0.5f) {
        float total = 0.0f;
        float maxValue = 0.0f;
        ...
        for (int i = 0; i < octaves; i++) {
            total += pnoise3d(x * frequency, y * frequency, z * frequency) * amplitude;
            maxValue += amplitude;
